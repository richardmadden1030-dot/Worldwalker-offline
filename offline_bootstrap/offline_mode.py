...
This module does not create a second ruleset. Living Adventures remains the
primary local resolver so offline actions advance the same calendar, training,
world clocks, economy, missions, travel, Chronicle and tactical combat used by
normal Worldwalker. This layer catalogs those actions into a player-friendly
...
def enabled(game):
    return bool(game.settings.get("offline_mode", True))
...
def _category_for(action_id):
    ...
    if action_id.startswith(("conflict:", "reputation:")): return "world"
...
def _travel_actions(state):
    ...
    for edge in edges[:12]:
        ...
        if route:
            rows.append(_decorate({"id":f"offline:travel:{route['id']}","label":f"Travel to {dest}","description":f"{route.get('label','Mapped route')} · {route.get('minutes',0)} min · exposure {route.get('exposure',0)}.","minutes":route.get("minutes",0),"destination":dest,"route_id":route["id"],"preparation":"normal"},"travel","offline_travel"))
    return rows
...
def _social_resolve(game,action_id):
    ...
    relation=state.setdefault("relationships",{}).setdefault(name,{"score":0})
    if not isinstance(relation,dict):relation=state["relationships"][name]={"score":0}
    goal=_text(available[name].get("goal"))
    if kind=="goals":
        text=f"You ask {name} what is occupying their attention. "+(f"Their established concern is: {goal}." if goal else "They have no concrete public request recorded right now.")
        return _local_turn(game,text,15)
    if kind=="train":
    ...
    gain=0 if count>=2 else (2 if kind=="help" else 1)
    label=(f"You spend an unhurried hour with {name}." if kind=="spend" else f"You help {name} make practical progress toward: {goal}.")
    result=_local_turn(game,label,60);relation["score"]=min(100,int(relation.get("score",0) or 0)+gain);ledger[key]=count+1
...
def opening(game):
    ...
    s["opening_complete"]=True;s["offline_mode"]=True
    local=location_view(s,place);services=", ".join(local.get("services",[])[:3]);lead=next((r for r in local.get("actions",[]) if r.get("id","").startswith(("mission:","expedition:","conflict:"))),None)
    ...
    narrative+=" Choose Activities to decide what happens next; the Chronicle records only outcomes the game actually resolves."
    game.append(narrative,"narrative",canon_day=s.get("canon_day"));game.autosave()
